const AI_KEY_STORAGE = 'html_editor_ai_key_groq';
const MAX_RETRIES = 2;
...
export class AIService {
  ...
  async findErrors(html: string, css: string, js: string): Promise<string> {
    const system = `Review HTML/CSS/JS code for bugs and errors. List each issue with: file, problem, and fix. Be concise. If no issues, say "No issues found."`;
    const user = `HTML:\n${this.trimCode(html)}\nCSS:\n${this.trimCode(css)}\nJS:\n${this.trimCode(js)}`;
